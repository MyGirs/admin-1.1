<template>
  ...
  >
    ...
    <el-form label-width="140" class="dialog-form">
      <el-col
        :xl="10"
        :lg="10"
        :md="12"
...
import { updateMeasureListApi, getMapApi } from "../api";
...
const handleUpdate = async () => {
  ...
  try {
    let res = await updateMeasureListApi(form.value);
...
const getMap = async () => {
  ...
  optionsList.value = getMajorConfig(data.rectificationMap);
};
onBeforeMount(async () => {
  await getMap();
});
